...
abstract class AbstractTermMap(
    ...
    val languageMap: Option[String],
    val nestedTermMap: Option[xR2RMLNestedTermMap],
    val refFormulation: String,
    
    /** listPushDown should be set only if there is a nested term map */
     ...
     *
     * If the node passed is not a resource but a literal, then it means that we have a constant property whose object
    ...
    def getDefaultTermType: String = {
        val result = this match {
            case _: R2RMLObjectMap => {
                if (termMapType == Constants.MorphTermMapType.ColumnTermMap
                    ...
                    || languageTag.isDefined || languageMap.isDefined
                    || datatype.isDefined) {
        ...
        trait CalculatedTermProperties {
          def getCollectionTermType: Option[String]
          def getTermType: String
          def getDatatype: Option[String]
          def getLanguageTag: Option[String]
          def getLanguageMap: Option[String]
    }
     ...
     * 
     * @return object containing the term type from the term map, datatype, language tag, language map, term type
     */
    def calculateCollecTermProperties(): CalculatedTermProperties = {
        ...
        if (AbstractTermMap.isRdfCollectionTermType(termMapTermType)) {
            if (this.nestedTermMap.isDefined) {
                ...
                val ntmLanguageMap = this.nestedTermMap.get.languageMap

                new CalculatedTermProperties {
                  def getCollectionTermType = Some(termMapTermType)
                  def getTermType = ntmTermType
                  def getDatatype = ntmDataType
                  def getLanguageTag = ntmLanguageTag
                  def getLanguageMap = ntmLanguageMap
                }
            } else {
                logger.warn("Term map with collection/container term type but no nested term map: " + this)
                val dataType = this.datatype;
                val languageTag = this.languageTag
                val languageMap = this.languageMap
                
                new CalculatedTermProperties {
                  def getCollectionTermType = Some(termMapTermType)
                  def getTermType = Constants.R2RML_LITERAL_URI
                  def getDatatype = dataType
                  def getLanguageTag = languageTag
                  def getLanguageMap = languageMap
                }
            }
            ...
            if (this.nestedTermMap.isDefined) {
                ...
                new CalculatedTermProperties {
                  def getCollectionTermType = None
                  def getTermType = ntmTermType
                  def getDatatype = ntmDataType
                  def getLanguageTag = ntmLanguageTag
                  def getLanguageMap = ntmLanguageMap
                ...
                new CalculatedTermProperties {
                  def getCollectionTermType = None
                  def getTermType = termMapTermType
                  def getDatatype = dataType
                  def getLanguageTag = languageTag
                  def getLanguageMap = languageMap
                }
    ...
    override def toString = s"AbstractTermMap(termMapType=$termMapType, termType=$termType, datatype=$datatype, languageTag=$languageTag, languageMap=$languageMap, nestedTermMap=$nestedTermMap, refFormulation=$refFormulation, listPushDown=$listPushDown)"
}
...
object AbstractTermMap {
    ...
    /**
     * Extract the value of the rr:termType property, returns None is no property found
     */
    ...
    def extractLanguageMapTag(rdfNode: RDFNode) = {
        rdfNode match {
            case resource: Resource => {
                val languageMapStatement = resource.getProperty(Constants.R2RML_LANGUAGE_MAP_PROPERTY);
                if (languageMapStatement != null) {
                    Some(languageMapStatement.getObject().toString());
                } else
                    None
            }
            case _ => None
    ...
    trait TermMapCoreProperties {
          def getTermMapType: Constants.MorphTermMapType.Value
          def getTermType: Option[String]
          def getDatatype: Option[String]
          def getLanguageTag: Option[String]
          def getLanguageMap: Option[String]
          def getNestedTM: Option[xR2RMLNestedTermMap]
          def getListPushDown: List[xR2RMLPushDown] 
    }
     ...
     * <li>optional list of "pushDown" properties</li>
     * </ul>
     */
    def extractCoreProperties(rdfNode: RDFNode, refFormulation: String) = {
        ...
        val languageMap = AbstractTermMap.extractLanguageMapTag(rdfNode);
        ...
        if (logger.isTraceEnabled()) logger.trace("Extracted term map core properties: termMapType: " + termMapType + ". termType: "
            + termType + ". datatype: " + datatype + ". languageTag: " + languageTag + ". languageMap: " + languageMap
            + ". nestedTermMap: " + nestedTM + ". listPushDown: " + listPushDown
        )

        val coreProperties = new TermMapCoreProperties {
          def getTermMapType: Constants.MorphTermMapType.Value = termMapType
          def getTermType = termType
          def getDatatype = datatype
          def getLanguageTag = languageTag
          def getLanguageMap = languageMap
          def getNestedTM = nestedTM
          def getListPushDown = listPushDown 
        }
        coreProperties;
    }
}