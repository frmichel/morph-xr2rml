...
class MorphMongoDataTranslator(val fact: IMorphFactory) extends MorphBaseDataTranslator(fact) with java.io.Serializable {
	...
	def translateData(termMap: AbstractTermMap, jsonDoc: String): List[RDFTerm] = {
		...
		//val termMapDatatype = termMap.datatype
  ...
  : List[RDFTerm] = {
    ...
    termMap.calculateCollectionTermnTypeAndDataTypeAndLanguageTagAndMemberTermType();
		...
		val listReplace = for (i <- 0 to (msPaths.length - 1)) yield {
			...
			valuesRaw.filter(_ != null).map(v => MorphBaseDataTranslator.encodeResvdCharsIfUri(
				v, memberTermType, encodeUnsafeCharsInDbValues))
		}
		...
		if (isEmptyReplacements) {
			if (logger.isTraceEnabled()) logger.trace("Template " + termMap.getTemplateString() + ": no values (or only null values) were read from from the DB.")
			List()
		} else {
			// Compute the list of template results by making all possible combinations of the replacement values
			val tplResults = TemplateUtility.replaceTemplateGroups(termMap.getTemplateString(), replacements);
			val resultAux =	MorphBaseDataTranslator.translateMultipleValues(tplResults, collecTermType
				, memberTermType, datatype, languageTag, encodeUnsafeCharsInUri, encodeUnsafeCharsInDbValues);
			resultAux
		}
	}
